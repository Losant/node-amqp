...
Exchange.prototype._onMethod = function (channel, method, args) {
  ...
  switch (method) {
    ...
    case methods.basicNack:
      var isNack = method === methods.basicNack;

      this.emit(isNack ? 'basic-nack' : 'basic-ack', args);
      var sequenceNumber = args.deliveryTag.readUInt32BE(4), tag;
      debug && debug(method.name, ", sequence: ", sequenceNumber);

      var emitName = isNack ? 'ack error' : 'ack';
      if (sequenceNumber === 0 && args.multiple === true) {
        // we must ack everything
        for (tag in this._unAcked) {
          this._unAcked[tag].emit(emitName);
        ...
        // simple single ack
        this._unAcked[sequenceNumber].emit(emitName);
